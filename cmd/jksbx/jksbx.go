...
func main() {
	...
	queueSize := flag.Int("q", 250, "申报请求的队列大小，默认250")
	concurrency := flag.Int("c", 5, "并发进行申报的协程数目，默认5")
	...
	if *queueSize <= 0 || *concurrency <= 0 {
		panic("队列大小和并发数目必须为正整数")
	}
	router.InitializeApiEndpoints(*headfulMode, *queueSize, *concurrency)
